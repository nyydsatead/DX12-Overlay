...
static void OutputHR(const char* op, HRESULT hr) {
    std::ostringstream ss;
    ss << op << " failed. " << HResultToString(hr) << "\n";
...
bool DX12Renderer::Initialize(HWND hwnd, bool transparentComposition) {
    ...
    if (FAILED(hr)) { OutputHR("CreateDXGIFactory2", hr); return false; }

    // Create device
    hr = D3D12CreateDevice(nullptr, D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&m_device));
    if (FAILED(hr)) { OutputHR("D3D12CreateDevice", hr); return false; }
    ...
    if (FAILED(hr)) { OutputHR("CreateCommandQueue", hr); return false; }
    ...
    // Descriptor heaps
    {
        ...
        if (FAILED(hr)) { OutputHR("CreateDescriptorHeap(RTV)", hr); return false; }
        m_rtvDescriptorSize = m_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

        // ImGui SRV heap: font + room for user textures
        D3D12_DESCRIPTOR_HEAP_DESC srvHeapDesc = {};
        srvHeapDesc.NumDescriptors = 64;
        srvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
        srvHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
        hr = m_device->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&m_srvHeap));
        if (FAILED(hr)) { OutputHR("CreateDescriptorHeap(SRV)", hr); return false; }
    }
    ...
    for (UINT i = 0; i < FRAME_COUNT; ++i) {
        ...
        if (FAILED(hr)) { OutputHR("CreateCommandAllocator", hr); return false; }
    }
    ...
    if (FAILED(hr)) { OutputHR("CreateCommandList", hr); return false; }
    m_commandList->Close();

    // Fence + event
    hr = m_device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_fence));
    if (FAILED(hr)) { OutputHR("CreateFence", hr); return false; }
...
bool DX12Renderer::InitForComposition(HWND hwnd, IDXGIFactory4* factory) {
    // DirectComposition path: premultiplied alpha BGRA is best supported
    m_backBufferFormat = DXGI_FORMAT_B8G8R8A8_UNORM;

    // Enable waitable swap chain for better pacing
    ...
    scDesc.AlphaMode = DXGI_ALPHA_MODE_PREMULTIPLIED; // key for per-pixel transparency
    scDesc.Flags = m_swapChainFlags;

    ComPtr<IDXGISwapChain1> sc1;
    HRESULT hr = factory->CreateSwapChainForComposition(m_commandQueue.Get(), &scDesc, nullptr, &sc1);
    if (FAILED(hr)) { OutputHR("CreateSwapChainForComposition", hr); return false; }

    hr = sc1.As(&m_swapChain);
    if (FAILED(hr)) { OutputHR("swapChain.As", hr); return false; }

    // Reduce latency and obtain waitable object (improves pacing and CPU cost)
    if (ComPtr<IDXGISwapChain2> sc2; SUCCEEDED(m_swapChain.As(&sc2)) && sc2) {
        sc2->SetMaximumFrameLatency(2); // try 2–3; 2 usually feels smooth for overlays
        m_frameLatencyWaitableObject = sc2->GetFrameLatencyWaitableObject();
    }

    // Create DirectComposition device/target/visual and attach swap chain
    hr = DCompositionCreateDevice(nullptr, __uuidof(IDCompositionDevice), reinterpret_cast<void**>(m_dcompDevice.GetAddressOf()));
    if (FAILED(hr)) { OutputHR("DCompositionCreateDevice", hr); return false; }

    hr = m_dcompDevice->CreateTargetForHwnd(hwnd, TRUE, &m_dcompTarget);
    if (FAILED(hr)) { OutputHR("IDCompositionDevice::CreateTargetForHwnd", hr); return false; }

    hr = m_dcompDevice->CreateVisual(&m_dcompVisual);
    if (FAILED(hr)) { OutputHR("IDCompositionDevice::CreateVisual", hr); return false; }

    hr = m_dcompVisual->SetContent(m_swapChain.Get());
    if (FAILED(hr)) { OutputHR("IDCompositionVisual::SetContent", hr); return false; }

    hr = m_dcompTarget->SetRoot(m_dcompVisual.Get());
    if (FAILED(hr)) { OutputHR("IDCompositionTarget::SetRoot", hr); return false; }

    hr = m_dcompDevice->Commit();
    if (FAILED(hr)) { OutputHR("IDCompositionDevice::Commit", hr); return false; }
...
bool DX12Renderer::InitForHwnd(HWND hwnd, IDXGIFactory4* factory) {
    ...
    if (FAILED(hr)) {
        ...
        if (FAILED(hr)) { OutputHR("CreateSwapChainForHwnd", hr); return false; }
...
void DX12Renderer::CreateRenderTargets() {
    ...
    for (UINT i = 0; i < FRAME_COUNT; ++i) {
        ...
        if (FAILED(hr)) { OutputHR("IDXGISwapChain::GetBuffer", hr); continue; }
...
void DX12Renderer::BeginFrame() {
    // Wait for swap chain frame latency object (improves pacing/CPU)
    if (m_frameLatencyWaitableObject) {
        WaitForSingleObject(m_frameLatencyWaitableObject, 1000);
    }

    // Get current back buffer and wait if still in use
    m_frameIndex = m_swapChain->GetCurrentBackBufferIndex();
    ...
    // Clear; for composition we want fully transparent background (premultiplied alpha)
...
void DX12Renderer::EndFrame() {
    ...
    // Present (full frame; partial present not supported with flip-model)
    UINT syncInterval = 0; // immediate
    UINT presentFlags = 0;
    ...
    // No per-frame DComp Commit needed; only on topology changes
